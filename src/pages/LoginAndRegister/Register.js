        ...
        createUser(email, password)
            .then(result => {
                ...
                fetch('http://localhost:5000/jwt', {
                    ...
                    .then(res => res.json())
                    .then(data => {
                        localStorage.setItem('exPhotography-token', data.token)
                ...
                <div className="w-full max-w-md p-8 space-y-3 rounded-xl bg-gray-300 text-gray-900">
                    ...
                    <form onSubmit={handelRegister} className="space-y-6 ng-untouched ng-pristine ng-valid">
                        <div className="space-y-1 text-sm">
                            ...
                            <input type="text" name="userName" id="userName" placeholder="User Name" className="w-full px-4 py-3 rounded-md border-gray-700 bg-gray-100 text-gray-900 focus:border-violet-400" />
                        </div>
                        <div className="space-y-1 text-sm">
                            <label htmlFor="email" className="block text-gray-900">Email</label>
                            <input type="email" name="email" id="email" placeholder="Email" className="w-full px-4 py-3 rounded-md border-gray-700 bg-gray-100 text-gray-900 focus:border-violet-400" />
                        </div>
                        <div className="space-y-1 text-sm">
                        ...
                        <button className="block w-full p-3 text-center font-bold rounded-sm text-gray-900 bg-cyan-400">Sign Up</button>
                    </form>
                    ...
                    <div className="flex justify-center space-x-4">
                        ...
                        <button onClick={handelGoogleLogin} aria-label="Log in with Twitter" className="p-3 rounded-sm">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" className="w-5 h-5 fill-current">
                        ...
                        <button onClick={handelGoogleLogin} aria-label="Log in with GitHub" className="p-3 rounded-sm">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" className="w-5 h-5 fill-current">
